...
4. **Run and fail over.** The CLI is started in the account's sandbox workspace. If it reports a rate limit, an auth error, a crash or a timeout **before producing any content**, the account is put in cooldown and the next candidate is tried (`x-cta-failovers` counts these). Once content has started streaming there is no failover.
5. **Errors** when nothing is left: `429` all accounts rate-limited (with `Retry-After`), `503` all slots busy, `502` auth/crash, `504` timeout, `400 tools_unsupported` when no target can bridge client tools.
